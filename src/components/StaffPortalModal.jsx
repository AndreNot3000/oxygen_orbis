...
export default function StaffPortalModal({ isOpen, onClose }) {
  ...
  const [walkInRoomType, setWalkInRoomType] = useState('deluxe-king');
  const [walkInUnitNumber, setWalkInUnitNumber] = useState('108');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#0B131B] border border-[#C5A880]/50 rounded-3xl max-w-6xl w-full max-h-[95vh] overflow-hidden shadow-2xl flex flex-col text-slate-100 relative">
        ...
        <div className="p-4 sm:p-5 border-b border-[#243546] bg-[#111B24] flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="hidden md:flex items-center gap-4 text-xs">
            <div className="px-3 py-1.5 rounded-xl bg-[#0A1118] border border-[#243546]">
              <span className="text-slate-400 block text-[9px] uppercase">Occupancy</span>
              <span className="font-bold text-emerald-400">{stats.occupancyPercentage}% ({stats.occupiedCount}/40)</span>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-[#0A1118] border border-[#243546]">
              <span className="text-slate-400 block text-[9px] uppercase">Available Clean</span>
              <span className="font-bold text-white">{stats.availableCount} Rooms</span>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-[#0A1118] border border-[#243546]">
              <span className="text-slate-400 block text-[9px] uppercase">Housekeeping</span>
              <span className="font-bold text-amber-400">{stats.dirtyCount} Pending</span>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-[#0A1118] border border-[#243546]">
              <span className="text-slate-400 block text-[9px] uppercase">Active Staff</span>
              <span className="font-bold text-[#C5A880]">Tola Adeyemi (Lead)</span>
            </div>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-full bg-[#172430] hover:bg-[#243546] text-slate-400 hover:text-white transition ml-auto"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-4 sm:px-6 py-2.5 bg-[#0A1017] border-b border-[#1E2C3B] flex items-center gap-2 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden text-xs">
          {[
            ...
            return (
              <button
                ...
                className={`py-2 px-3.5 rounded-xl font-semibold flex items-center gap-2 whitespace-nowrap transition-all duration-200 ${
                  isActive
                    ? 'bg-gradient-to-r from-[#C5A880] via-[#D4B98E] to-[#DFC69C] text-black font-bold shadow-md shadow-[#C5A880]/20 scale-[1.02]'
                    : 'bg-[#111A23] text-slate-300 hover:text-white hover:bg-[#182430] border border-[#223142]'
                }`}
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-black' : 'text-[#C5A880]'}`} />
                <span>{tab.label}</span>
                {tab.badge !== undefined && (
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold border ${isActive ? 'bg-black/20 text-black border-black/30' : tab.badgeColor}`}>
                    {tab.badge}
                  </span>
                )}
              </button>
        ...
        {/* Tab Body */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#070D13]">
          ...
          {activeTab === 'timeline' && (
            <div className="space-y-4">
              ...
              <div className="p-3 bg-[#0D151F] border border-[#1E2D3E] rounded-2xl flex flex-wrap items-center justify-between gap-3 shadow-lg">
                {/* Left: Floor Segmented Pills */}
                <div className="flex items-center gap-1.5 bg-[#080E15] p-1 rounded-xl border border-[#1E2D3E] text-xs">
                  <span className="text-[11px] text-slate-400 font-bold px-2">Floor:</span>
                  {[
                    { id: 'ALL', label: 'All (40)' },
                    { id: '1', label: 'F1 • Deluxe' },
                    { id: '2', label: 'F2 • Deluxe' },
                    { id: '3', label: 'F3 • Exec' },
                    { id: '4', label: 'F4 • Penthouse' },
                  ].map((f) => (
                    <button
                      key={f.id}
                      type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#111B24] border border-[#243546] rounded-xl px-3 py-2.5 text-white"
                    >
                      <option value="deluxe-king">Deluxe King (₦48,000/night)</option>
                      <option value="executive-room">Executive Suite (₦72,000/night)</option>
                      <option value="presidential-suite">Presidential Penthouse (₦145,000/night)</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#111B24] border border-[#243546] rounded-xl px-3 py-2.5 text-white font-mono"
                    >
                      {rooms
                        .filter((r) => r.status === 'AVAILABLE' && r.housekeeping === 'INSPECTED_CLEAN')
                        .map((r) => (
                          <option key={r.unitNumber} value={r.unitNumber}>
                            Room {r.unitNumber} (Floor {r.floor} • Clean)
                          </option>
                        ))}
                    </select>
                  </div>
